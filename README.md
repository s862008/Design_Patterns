...
– “Прототип”: этот шаблон используется для клонирования объектов.
     ...
     Паттерн «Прототип» (Prototype) в Java позволяет создавать новые объекты путём клонирования существующего экземпляра (прототипа), а не через конструктор. Это полезно, когда создание объекта ресурсоёмко или требует сложной инициализации.
